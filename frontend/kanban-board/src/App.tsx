...
function DetailPanel({
  card, project, columnsList, members, onClose, onChanged, onDelete,
}: {
  card: Card;
  project: Project;
  columnsList: BoardColumn[];
  ...
  const addComment = async () => {
    ...
    const created = await kanbanService.addComment(card.id, 'm1', commentText.trim());
...
function App() {
  ...
  const [composer, setComposer] = useState<{ open: boolean; columnId: string }>({ open: false, columnId: 'c1' });
  ...
  useEffect(() => { void reloadBoard(true); }, [activeProjectId, filters.search, filters.assigneeId, filters.due, filters.priorities.join(',')]);
